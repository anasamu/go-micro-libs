...
func (p *Provider) GetSupportedFeatures() []messaging.MessagingFeature {
	return []messaging.MessagingFeature{
		...
		messaging.FeatureRequestReply,
		messaging.FeatureMessageRouting,
		messaging.FeatureMessageFiltering,
		...
		messaging.FeatureMessageDeadLetter,
		messaging.FeatureMessageScheduling,
		messaging.FeatureMessagePriority,
		messaging.FeatureMessageTTL,
...
func (p *Provider) ListTopics(ctx context.Context) ([]messaging.TopicInfo, error) {
	...
	topics := make([]messaging.TopicInfo, 0, len(result.QueueUrls))
...
func (p *Provider) handleMessages(ctx context.Context, queueURL string, request *messaging.SubscribeRequest, handler messaging.MessageHandler) {
	// Receive messages
...
func (p *Provider) handleMessage(ctx context.Context, message types.Message, handler messaging.MessageHandler) {
	var messagingMessage messaging.Message
	if err := json.Unmarshal([]byte(*message.Body), &messagingMessage); err != nil {
