...
func (p *Provider) ListTopics(ctx context.Context) ([]messaging.TopicInfo, error) {
	...
	topics := make([]messaging.TopicInfo, 0)
...
func (p *Provider) handleMessage(ctx context.Context, delivery amqp091.Delivery, handler messaging.MessageHandler) {
	var messagingMessage messaging.Message
	if err := json.Unmarshal(delivery.Body, &messagingMessage); err != nil {
