...
func (kp *KubernetesProvider) GetHealth(ctx context.Context, serviceID string) (types.HealthStatus, error) {
	...
	if readyEndpoints > 0 {
		return types.HealthPassing, nil
	}

	return types.HealthCritical, nil
}
		...
		for {
			...
			case event := <-watch.ResultChan():
				...
				serviceEvent := &types.ServiceEvent{
					Type:      convertKubernetesEventType(string(event.Type)),
					Timestamp: time.Now(),
					Provider:  kp.GetName(),
				}

				// Get the full service details
				serviceObj, err := kp.GetService(ctx, service.Name)
				if err != nil {
					kp.logger.WithError(err).WithField("service", service.Name).Warn("Failed to get service details")
					continue
				}
