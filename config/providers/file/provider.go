...
func (p *Provider) Save(config *types.Config) error {
	...
	writeViper.Set("config", config.Configuration)
	...
	writeViper.Set("api", config.API)
	writeViper.Set("communication", config.Communication)
	writeViper.Set("cache", config.Cache)
	writeViper.Set("messaging", config.Messaging)
	writeViper.Set("email", config.Email)
	writeViper.Set("payment", config.Payment)
	writeViper.Set("discovery", config.Discovery)
	writeViper.Set("failover", config.Failover)
	writeViper.Set("edge", config.Edge)
	writeViper.Set("circuitbreaker", config.CircuitBreaker)
	writeViper.Set("event", config.Event)
	writeViper.Set("ratelimit", config.RateLimit)
	writeViper.Set("scheduling", config.Scheduling)
	writeViper.Set("zerotrust", config.ZeroTrust)
	writeViper.Set("backup", config.Backup)
	writeViper.Set("chaos", config.Chaos)
	writeViper.Set("ai", config.AI)
	writeViper.Set("middleware", config.Middleware)
	writeViper.Set("filegen", config.FileGen)
