...
func (p *Provider) GetSupportedFeatures() []storage.StorageFeature {
	return []storage.StorageFeature{
		storage.FeaturePresignedURLs,
		storage.FeaturePublicURLs,
		storage.FeatureMultipart,
		storage.FeatureVersioning,
		storage.FeatureEncryption,
		storage.FeatureLifecycle,
		storage.FeatureCORS,
	}
}
...
func (p *Provider) PutObject(ctx context.Context, request *storage.PutObjectRequest) (*storage.PutObjectResponse, error) {
	if !p.IsConfigured() {
	...
	response := &storage.PutObjectResponse{
		Key:          request.Key,
...
func (p *Provider) GetObject(ctx context.Context, request *storage.GetObjectRequest) (*storage.GetObjectResponse, error) {
	if !p.IsConfigured() {
		return nil, fmt.Errorf("minio provider not configured")
	}
	...
	response := &storage.GetObjectResponse{
		Content:      object,
		Size:         info.Size,
...
func (p *Provider) DeleteObject(ctx context.Context, request *storage.DeleteObjectRequest) error {
	if !p.IsConfigured() {
...
func (p *Provider) DeleteObjects(ctx context.Context, request *storage.DeleteObjectsRequest) (*storage.DeleteObjectsResponse, error) {
	if !p.IsConfigured() {
		return nil, fmt.Errorf("minio provider not configured")
	}
	...
	// Delete objects
	response := &storage.DeleteObjectsResponse{
		Deleted: make([]storage.DeletedObject, 0, len(request.Keys)),
		Errors:  make([]storage.DeleteError, 0),
	}

	for err := range p.client.RemoveObjects(ctx, request.Bucket, objectsCh, minio.RemoveObjectsOptions{}) {
		if err.Err != nil {
			response.Errors = append(response.Errors, storage.DeleteError{
				Key:     err.ObjectName,
				Code:    "DeleteFailed",
				Message: err.Err.Error(),
			})
		} else {
			response.Deleted = append(response.Deleted, storage.DeletedObject{
				Key: err.ObjectName,
			})
		}
	}
...
func (p *Provider) ListObjects(ctx context.Context, request *storage.ListObjectsRequest) (*storage.ListObjectsResponse, error) {
	if !p.IsConfigured() {
		return nil, fmt.Errorf("minio provider not configured")
	}
...
func (p *Provider) ObjectExists(ctx context.Context, request *storage.ObjectExistsRequest) (bool, error) {
	if !p.IsConfigured() {
...
func (p *Provider) CopyObject(ctx context.Context, request *storage.CopyObjectRequest) (*storage.CopyObjectResponse, error) {
	if !p.IsConfigured() {
		return nil, fmt.Errorf("minio provider not configured")
	}
	...
	response := &storage.CopyObjectResponse{
		Key:          request.DestKey,
		ETag:         strings.Trim(result.ETag, "\""),
		LastModified: result.LastModified,
		Size:         result.Size,
...
func (p *Provider) MoveObject(ctx context.Context, request *storage.MoveObjectRequest) (*storage.MoveObjectResponse, error) {
	// Move is implemented as copy + delete
	copyRequest := &storage.CopyObjectRequest{
	...
	deleteRequest := &storage.DeleteObjectRequest{
		Bucket: request.SourceBucket,
	...
	response := &storage.MoveObjectResponse{
...
func (p *Provider) GetObjectInfo(ctx context.Context, request *storage.GetObjectInfoRequest) (*storage.ObjectInfo, error) {
	if !p.IsConfigured() {
		return nil, fmt.Errorf("minio provider not configured")
	}
...
func (p *Provider) GeneratePresignedURL(ctx context.Context, request *storage.PresignedURLRequest) (string, error) {
	if !p.IsConfigured() {
		return "", fmt.Errorf("minio provider not configured")
	}
...
func (p *Provider) GeneratePublicURL(ctx context.Context, request *storage.PublicURLRequest) (string, error) {
	if !p.IsConfigured() {
		return "", fmt.Errorf("minio provider not configured")
	}
...
func (p *Provider) CreateBucket(ctx context.Context, request *storage.CreateBucketRequest) error {
	if !p.IsConfigured() {
		return fmt.Errorf("minio provider not configured")
	}
...
func (p *Provider) DeleteBucket(ctx context.Context, request *storage.DeleteBucketRequest) error {
	if !p.IsConfigured() {
		return fmt.Errorf("minio provider not configured")
	}
...
func (p *Provider) BucketExists(ctx context.Context, request *storage.BucketExistsRequest) (bool, error) {
	if !p.IsConfigured() {
		return false, fmt.Errorf("minio provider not configured")
	}
...
func (p *Provider) ListBuckets(ctx context.Context) ([]storage.BucketInfo, error) {
	if !p.IsConfigured() {
		return nil, fmt.Errorf("minio provider not configured")
	}
	...
	// Prepare response
	bucketInfos := make([]storage.BucketInfo, len(buckets))
	for i, bucket := range buckets {
		bucketInfos[i] = storage.BucketInfo{
			Name:         bucket.Name,
