...
func (p *Provider) DeleteObjects(ctx context.Context, request *storageProvider.DeleteObjectsRequest) (*storageProvider.DeleteObjectsResponse, error) {
	...
	response := &storageProvider.DeleteObjectsResponse{
		Deleted: make([]storageProvider.DeletedObject, 0, len(request.Keys)),
		Errors:  make([]storageProvider.DeleteError, 0),
	}
	...
	for _, key := range request.Keys {
		...
		if err != nil {
			response.Errors = append(response.Errors, storageProvider.DeleteError{
				Key:     key,
				Code:    "DeleteFailed",
				Message: err.Error(),
			})
		} else {
			response.Deleted = append(response.Deleted, storageProvider.DeletedObject{
				Key: key,
			})
		}
	}
...
func (p *Provider) ListObjects(ctx context.Context, request *storageProvider.ListObjectsRequest) (*storageProvider.ListObjectsResponse, error) {
	...
	response := &storageProvider.ListObjectsResponse{
		Objects:        make([]storageProvider.ObjectInfo, 0),
	...
	for {
		...
		// Add object info
		objInfo := storageProvider.ObjectInfo{
			Key:          attrs.Name,
			Size:         attrs.Size,
			LastModified: attrs.Updated,
			ETag:         hex.EncodeToString(attrs.MD5),
...
func (p *Provider) ListBuckets(ctx context.Context) ([]storageProvider.BucketInfo, error) {
	...
	buckets := make([]storageProvider.BucketInfo, 0)
	for {
		attrs, err := it.Next()
		if err == iterator.Done {
			break
		...
		buckets = append(buckets, storageProvider.BucketInfo{
			Name:         attrs.Name,
