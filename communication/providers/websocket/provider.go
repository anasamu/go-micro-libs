...
type Provider struct {
	...
	mu         sync.RWMutex
	stats      *communication.CommunicationStats
}
...
func (p *Provider) GetSupportedFeatures() []communication.CommunicationFeature {
	return []communication.CommunicationFeature{
		communication.FeatureWebSocket,
		communication.FeatureRealTime,
		communication.FeatureBroadcasting,
		communication.FeatureMulticasting,
		communication.FeatureUnicasting,
		communication.FeatureAuthentication,
		communication.FeatureRateLimiting,
		communication.FeatureMetrics,
		communication.FeatureLogging,
		communication.FeatureHealthChecks,
		communication.FeatureMiddleware,
		communication.FeatureHeaders,
		communication.FeatureQueryParams,
		communication.FeatureBodyParsing,
		communication.FeatureStreaming,
	}
}

// GetConnectionInfo returns connection information
func (p *Provider) GetConnectionInfo() *communication.ConnectionInfo {
	...
	return &communication.ConnectionInfo{
		ID:          "websocket-server",
		Type:        "websocket",
...
func (p *Provider) HandleRequest(ctx context.Context, request *communication.Request) (*communication.Response, error) {
	return nil, fmt.Errorf("HTTP requests not supported by WebSocket provider")
}

// HandleWebSocket handles a WebSocket connection
func (p *Provider) HandleWebSocket(ctx context.Context, request *communication.WebSocketRequest) (*communication.WebSocketResponse, error) {
	if !p.IsRunning() {
	...
	response := &communication.WebSocketResponse{
...
func (p *Provider) SendMessage(ctx context.Context, request *communication.SendMessageRequest) (*communication.SendMessageResponse, error) {
	if !p.IsRunning() {
		return nil, fmt.Errorf("websocket server not running")
	}
	...
	case targetClient.send <- data:
		...
		response := &communication.SendMessageResponse{
...
func (p *Provider) BroadcastMessage(ctx context.Context, request *communication.BroadcastRequest) (*communication.BroadcastResponse, error) {
	if !p.IsRunning() {
		return nil, fmt.Errorf("websocket server not running")
	}
	...
	response := &communication.BroadcastResponse{
...
func (p *Provider) GetConnections(ctx context.Context) ([]communication.ConnectionInfo, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()

	connections := make([]communication.ConnectionInfo, 0, len(p.clients))
	for client := range p.clients {
		connInfo := communication.ConnectionInfo{
			ID:          client.ID,
			Type:        "websocket",
...
func (p *Provider) GetStats(ctx context.Context) (*communication.CommunicationStats, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (c *Client) handleMessage(message *communication.Message) {
	switch message.Type {
	case "ping":
		// Respond with pong
		pongMessage := &communication.Message{
	...
	case "broadcast":
		...
		c.server.BroadcastMessage(context.Background(), &communication.BroadcastRequest{
			Message: message,
		})
	...
	case "private":
		...
		if message.To != "" {
			c.server.SendMessage(context.Background(), &communication.SendMessageRequest{
				UserID:  message.To,
				Message: message,
			})
		}
